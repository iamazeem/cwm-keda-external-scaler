...
server by the [logger](https://github.com/iamAzeem/fluent-plugin-http-cwm). The
Redis server stores a number of metrics. Based on its local configuration, the
external scaler fetches only the required one(s) to make scaling decisions.
